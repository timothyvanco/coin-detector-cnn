...
input_image = 'images/input_images/input_eu.jpg'
...
Material = Enum(('Copper_eu', 'Brass_eu', 'Euro1', 'Euro2'))
...
filename = 'classifier_RGB_eu.sav'
...
if materials[index_diameter] == "Euro2":
...
elif materials[index_diameter] == "Copper_eu":
    diameter = [x / biggestDiameter * cent5 for x in diameter]
    scaledTo = "Scaled to 5 euro"
...
while i < len(diameter):
    ...
    # eura
    if math.isclose(d, euro2, abs_tol=abs_euro2) and material == "Euro2":
        text = "2 Eura"
        total_value_eu += 200
...
cv2.putText(outputImage, scaledTo, (5, outputImage.shape[0] - 140), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255),
            lineType=cv2.LINE_AA)
cv2.putText(outputImage, "Coins detected: {}, money value: {:0.2f} eur".format(count, total_value_eu),
...
print("Coins detected: {}, money value: {:0.2f} eur".format(count, total_value_eu))
...
cv2.imwrite("images/output_images/output_image_eu.jpg", outputImage)
cv2.waitKey(0)
